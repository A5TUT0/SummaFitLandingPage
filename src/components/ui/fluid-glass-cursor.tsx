...
function FluidLens({ pointer, reduceMotion }: { pointer: React.RefObject<PointerPosition>; reduceMotion: React.RefObject<boolean> }) {
  ...
  useFrame((state, delta) => {
    ...
    if (reduceMotion.current) {
      lens.current.position.set(targetX, targetY, 15);
    } else {
...
export function FluidGlassCursor() {
  ...
  useEffect(() => {
    ...
    const updateMotionPreference = () => {
      ...
      if (reducedMotion.matches) pointer.current = { x: 0, y: 0 };
    };
    ...
    >
      ...
      >
        <Suspense fallback={null}>
          <FluidLens pointer={pointer} reduceMotion={reduceMotion} />
        </Suspense>
      </Canvas>
    </div>
  );
