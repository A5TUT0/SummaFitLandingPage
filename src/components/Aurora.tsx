...
export default function Aurora({
  colorStops = ["#4EDE66", "#BD68FE", "#32B0FE"],
  amplitude = 1,
  blend = 0.5,
  speed = 1,
  className = "",
  ...
  useEffect(() => {
    ...
    const toColorArray = (stops: [string, string, string]) =>
      stops.map((hex) => {
    ...
    const render = (time: number) => {
      ...
      program.uniforms.uTime.value =
        (reduceMotion ? 1200 : time) * 0.001 * current.speed;
      ...
      if (!hasSignaledReady) {
        ...
        document.documentElement.dataset.auroraReady = "true";
        window.dispatchEvent(new Event("summafit:aurora-ready"));
      }
